...
import { faArrowUpRightFromSquare, faArrowDown, faHandSpock, faHandsClapping } from '@fortawesome/free-solid-svg-icons';
    ...
    return (
        <motion.div 
            ...
            className={
             ...
             <motion.div className={`absoluste flex items-center justify-center left-0 top-0 w-[50px] h-[50px] rounded-full transition-all duration-500 opacity-0 ${(pageData.cursorState == "projects") ? 'scale-150 opacity-100 bg-blue-700' : null} ${(pageData.cursorState == "links") ? 'scale-150 opacity-100 bg-rose-700' : null} ${(pageData.cursorState == "images") ? 'scale-150 opacity-100 bg-indigo-700' : null} ${(pageData.cursorState == "thanks") ? 'scale-150 opacity-100 bg-pink-500' : null}`}>
